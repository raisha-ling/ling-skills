...
description: >-
  ...
  Amplitude, shows the trend, and breaks the change down by segment — acquisition
...
**read-only** — reads Amplitude, writes nothing. Produces an explanation and a
chart link; never modifies data or sends anything.
...
4. **Segment the change.** Break the metric down by the segments that explain
   movement — typically acquisition channel, platform, country, language, plan,
